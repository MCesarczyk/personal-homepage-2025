...
import { type Project, type ProjectImage } from "./types";
...
export const ProjectModal = ({ isOpen, onClose, onSave, onUpdate, project }: ProjectModalProps) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={project ? "Edit Project" : "Add New Project"} size="xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="codeUrl" className="block text-sm font-medium text-gray-700 mb-2">
            Code Repository URL
          </label>
          <input
            type="url"
          ...
          />
          {errors.codeUrl && <p className="mt-1 text-sm text-red-600">{errors.codeUrl}</p>}
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {formData.images.map((image) => (
                <div key={image.id} className="relative group">
                  <img src={image.url} alt="Project" className="w-full h-24 object-cover rounded-lg" />
                  <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg flex items-center justify-center">
                    <div className="opacity-0 group-hover:opacity-100 flex space-x-2">
                      ...
                      <Button type="button" size="sm" variant="danger" onClick={() => removeImage(image.id)}>
                        <Trash2 className="w-3 h-3" />
                      </Button>
                    </div>
                  </div>
                  {image.isCover && (
                    <div className="absolute top-2 left-2 bg-blue-600 text-white text-xs px-2 py-1 rounded">Cover</div>
                  )}
                </div>
              ))}
            </div>
